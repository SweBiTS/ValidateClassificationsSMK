...
temp_masked_fasta = snk_output.masked_fasta # k2mask output goes here
log_file = snk_log.path
k2mask_executable = snk_params.k2mask_exec
...
logging.info(f"k2mask Executable: {k2mask_executable}")
logging.info(f"Wildcards: {snk_wildcards}")
logging.info("----------------------------")

# --- Step 1: Run k2mask to generate lowercase masked fasta ---
logging.info(f"Running k2mask on {input_fasta}...")
cmd_k2mask = [
    k2mask_executable,
    "-in", str(input_fasta),
    "-outfmt", "fasta",
...
try:
    ...
    with open(log_file, "a") as log_f:
        process = subprocess.run(cmd_k2mask, check=True, text=True, encoding='utf-8',
                                 stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        log_f.write("\n--- k2mask stdout ---\n")
        log_f.write(process.stdout)
        log_f.write("\n--- k2mask stderr ---\n")
        log_f.write(process.stderr)
    logging.info("k2mask completed successfully.")

except subprocess.CalledProcessError as e:
    logging.error(f"k2mask failed with exit code {e.returncode}")
    if e.stdout: logging.error(f"k2mask stdout:\n{e.stdout}")
    if e.stderr: logging.error(f"k2mask stderr:\n{e.stderr}")
    sys.exit(1)
except FileNotFoundError:
    logging.error(f"Error: '{k2mask_executable}' command not found. Check KRAKEN2_BIN_DIR or PATH.")
    sys.exit(1)
except Exception as e:
    logging.error(f"An unexpected error occurred running k2mask: {e}", exc_info=True)
    sys.exit(1)


# --- Step 2: Parse lowercase masked fasta to BED ---
logging.info(f"Parsing {temp_masked_fasta} to generate BED file {output_bed}")
lc_pattern = re.compile(r"[a-z]+") # Find runs of lowercase letters (k2mask masks by lowercasing)
